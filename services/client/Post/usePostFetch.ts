...
const push = ()=>posts.value?.push({userId: 999999, id: 999999, title: "test", body: "test"})

const fetchPosts = async (): Promise<void> => {
    //key操作が関数ラップすると refresh, clearが反映されないので明示的にkeyを指定するAsyncDataじゃなくて、useFetchで良いかな
  const { data, error } = await useAsyncData<PostType[]>('posts' ,()=> $fetch("https://jsonplaceholder.typicode.com/posts"), 
